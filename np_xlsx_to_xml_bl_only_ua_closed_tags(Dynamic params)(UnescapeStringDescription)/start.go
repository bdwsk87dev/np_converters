...
func main() {
	...
	for _, row := range rows[1:] {
		...
		if len(row) > 51 && row[53] != "NULL" {
			offer.Param = append(offer.Param, Param{Name: row[51], Unit: "", Value: row[53]})
		}

		if len(row) > 55 && row[57] != "NULL" {
			offer.Param = append(offer.Param, Param{Name: row[55], Unit: "", Value: row[57]})
		}
